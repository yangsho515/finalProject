...
<div class="row g-4">
	<div class="bg-light rounded h-100 p-4">
		...
		<!-- 검색시작  -->
         ...
         <!-- 검색 끝  -->
         <form id="yearMonthForm" method="get" action="/employeeSalary/salaryList">
		    <!-- 연도 선택 -->
		    <label for="yearSelector" id="yearSelector">연도:</label>
		    <select id="year" name="year">
		        <c:forEach var="year" begin="2024" end="2025">
	            	<option value="${year}" <c:if test ="${year1 eq  year}">selected="selected"</c:if>>${year}</option>
		        </c:forEach>
		    </select>
		    ...
		    <select id="month" name="month">
		        <c:forEach var="month" begin="1" end="12" >
	            	<option value="${month}" <c:if test ="${month1 eq month}">selected="selected"</c:if>>${month}월</option>
		        </c:forEach>
		    </select>    
...
<!-- 		    <button class="btn btn-outline-dark m-2" type="button" onclick="loadSalariesByYearMonth()" id="loadSalariesByYearMonth">조회</button> -->
		    <button class="btn btn-outline-dark m-2" type="submit" id="loadSalariesByYearMonth">조회</button>
		</form>
	...
	<div class="bg-light rounded h-100 p-4">
		...
		<!-- <form id="monthSelectorForm">
		    ...
		    <select id="yearSelector" name="year" onchange="loadSalaryForYear()">
		        <option type="date" value="" id="year">연 선택</option>
		    </select>
		</form> -->
		...
		<form id="salaryForm" method="get" action="/employeeSalary/salaryList">
			...
			<div class="mb-3">
				<label for="papago" class="form-label">정산일</label> 
			...
			<button type="button" class="btn btn-outline-primary m-2" id="btnSubmit">등록</button>
			<button type="button" class="btn btn-outline-danger m-2" onclick="clearForm()">취소</button>
			<button type="button" class="btn btn-outline-secondary m-2" id="calculateBtn" style="float: right;">계산하기</button>
		</div>
	</div>
</div>	
...
<script type="text/javascript">
	...
	function calculateSalary() {
	    ...
	    let foodExpense = parseFloat($('#foodExpense').val());
			...
			*/
			...
			$.ajax({
				...
				success:function(result){
					...
					//등록
					if(result==1){
						location.href="/employeeSalary/salaryList?empId="+empId;
					}
				},
				error: function(xhr, status, error){
		...
		function modalList(currentPage, keyword){
		 	...
		 	$.ajax({
		 		...
		 		success:function(resp){
		 			...
		 			let str = "";
		 			$.each(resp.content,function(idx, employeeSalaryVO){
		 				str += `<tr>
							<td>\${employeeSalaryVO.rnum}</td>
							<td>\${employeeSalaryVO.empId}</td>
							<td>\${employeeSalaryVO.empName}</td>
		 			...
		 			//모달페이징 블록 처리
					$("#centerPagingArea").html(resp.pagingArea);
