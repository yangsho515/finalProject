...
<!-- 헤더 include  -->
	...
	<div class="bg-light text-center rounded p-4">
		  ...
		  <div class="container">
          <div class="hdFlex">
	         <h3 class="contlist">신고대기</h3>
	      </div>
        <!-- 검색 폼 -->
        <form action="/system/systemReportPost" method="post" class="search-form">
	        ...
	        <div class="nav nav-tabs custom-tabs">
                      ...
                      <tr>
                          ...
                          <div class="form-row">
				                <div class="form-group">
				                    ...
				                    <option value="${report.bodTypeName}">게시판 종류 선택하세요</option>
				                    <c:forEach var="report" items="${bodTypeNameList}">
				                       <option value="${report.bodTypeName}">${report.bodTypeName}</option>
				                    </c:forEach>   
				                    </select>
				                </div>
			               </div>
                          ...
                          <div class="form-row">
				                <div class="form-group">
				                    ...
				                    <option value="${systemReportVO.repTypeName}">신고 종류 선택하세요</option>
              ...
              <div class="d-flex mb-2">
              	<input class="form-control bg-transparent" name="keyword" type="text" placeholder="검색하세요">
              	<button type="button" class="btn btn-primary ms-2" id="btnSearch">검색</button>
                    ...
                    <div class="table-responsive">
                        ...
                        <%-- <c:forEach var="systemReportVO" items="${reportVOList}" varStatus="stat">
                                <tr>
                                    ...
                                    <td>${systemReportVO.bodTypeName}</td>
                                    <td>${systemReportVO.repTypeName}</td>
                                    <td><a href="/board/detail/${systemReportVO.bodNo}">${systemReportVO.bodTitle}</a></td>
                                    <td>${systemReportVO.repReason}</td>
                                    <td>${systemReportVO.repWriter}</td>
                                    <td>${systemReportVO.rnum}</td>
                                    <td>${systemReportVO.repDate}</td>
                                    <td>
	                                    <span id="spn1-${systemReportVOrepNo}" class="justify-between">
				                        <button 
				                        	class="btn btn-primary m-2" 
			                        		data-bod-no="${systemReportVOrepNo}"
					                        data-bod-type="${systemReportVO.bodTypeName}" id="edit" >
					                        대기
				                    	</button>
			                        </span>   
			                        <span id="spn2-${systemReportVOrepNo}" class="justify-between">
				                        <button 
				                        	class="btn btn-primary m-2" style="display:none;" 
			                        		data-bod-no="${systemReportVOrepNo}"
					                        data-bod-type="${systemReportVO.bodTypeName}"id="edit2"
					                         >
				                        완료
				                    	</button>
			                        </span>     
                    ...
                    <!-- 페이징 처리  -->
                    <div class="" id="divPagingArea" style="width: 30%; margin: 0 auto;"></div>
...
<!-- ///// footer 끝 ///// -->  
 	...
 	//신고 목록 조회
 	function reportList(currentPage,bodTypeName,repTypeName,keyword){
	 	...
	 	$.ajax({
	 		...
	 		success:function(articlePage){
	 			...
	 			//		List를                 0    vo 1개
	 			//		List를				  1    vo 1개
	 			$.each(articlePage.content,function(idx, systemReportVO){
	 				str += `<tr>
			 					<td>\${systemReportVO.rnum}</td>
		                        <td>\${systemReportVO.bodTypeName}</td>
		                        <td>\${systemReportVO.repTypeName}</td>
		                        <td style="text-align: left; color: #3676EB;"><a href="/board/detail/\${systemReportVO.bodNo}">\${systemReportVO.bodTitle}</a></td>
		                        <td>\${systemReportVO.repReason}</td>
		                        <td>\${systemReportVO.repWriter}</td>
		                        <td>\${systemReportVO.bodWriter}</td>
		                        <td>\${systemReportVO.repDate}</td>
		                        <td>
		                        <span id="spn1-\${systemReportVO.repNo}" class="justify-between">
			                        <button id="edit1-\${systemReportVO.repNo}"
			                        	class="btn btn-primary m-2" 
		                        		data-rep-no="\${systemReportVO.repNo}">
				                        대기
			                    	</button>
		                        </span>   
		                        </td>
 		...
 		$(function(){
 			reportList("1","","","");
 	 		...
 	 		$("#btnSearch").on("click",function(){
 	 			...
 	 			reportList("1",bodTypeName,repTypeName,keyword);
 	 		});
 		...
 		$(document).on("click", "[id^=edit1-]", function () {
	 	    let repNo = $(this).data("repNo"); // 버튼의 data-rep-no 값을 가져옴
	 	    console.log("클릭된 repNo: ", repNo);
	 	    ...
	 	    $.ajax({
	 	        ...
	 	        success: function (result) {
	 	            ...
	 	            if (result == 1) {
	 	                location.href = "/system/reportEnd";
	 	            }
	 	        }
	 	    });
	 	 	//1. 페이지 로딩 시 함수 호출(처음)
