		...
		<form id="frm" action="/sign/createFormPost" method="post">
			...
			<select class="form-select form-select-sm mb-3" aria-label=".form-select-sm example" id="appDocName">
				    ...
				    <div class="modal-dialog">
				        <div class="modal-content">
				            <div class="modal-header" style="background:white;">
				                <h5 class="modal-title" id="exampleModalLabel">결재 종류 선택</h5>
				                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
				            </div>
				            <div class="modal-body" id="modalContent">
				                <div id="modalContent"></div> <!-- 선택된 값이 여기에 표시됩니다. -->
				            </div>
				            <div class="modal-footer">
				                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">닫기</button>
				                <button type="button" class="btn btn-primary" onclick="downloadPDF()">PDF 저장</button>
				            </div>
				        </div>
				    </div>
				</div>
             ...
             <div class="form-floating mb-3">
                <input type="text" class="form-control" id="docTitle" value="${signVO.docTitle}" placeholder="제목" style="color:#ccc;height: 30px;">
            ...
            <div>
 			...
 			<button type="button" id="btnSubmit" class="btn btn-outline-primary m-2" style="float: right;">등록</button>
        </div>			
 	</form>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js"></script>
		...
		$(document).ready(function() {
			...
			$('#appDocName').on('change', function () {
		        const selectedValue = $(this).val();
		        const $modalContent = $("#modalContent");
		        if (selectedValue) {
		            $('#modalContent').text(`선택된 결재 종류: ${selectedValue}`);

		            let formHtml = '';
		            if (selectedValue === '품의서') {
		                formHtml = `
		                	<h6>1. 작성자</h6>
		                    <div class="mb-3">
		                        <label for="department" class="form-label">소속</label>
		                        <input type="text" class="form-control" name="department" placeholder="소속을 입력하세요">
		                    </div>
		                    <div class="mb-3">
		                        <label for="name" class="form-label">성명</label>
		                        <input type="text" class="form-control" name="name" placeholder="성명을 입력하세요">
		                    </div>
		                    <div class="mb-3">
		                        <label for="position" class="form-label">직급</label>
		                        <input type="text" class="form-control" name="position" placeholder="직급을 입력하세요">
		                    </div>
		                    <div class="mb-3">
		                        <label for="writeDate" class="form-label">작성일</label>
		                        <input type="date" class="form-control" name="writeDate" value="${signVO.writeDate}">
		                    </div>
							<hr/>
		                    <h6>2. 품의 내용</h6>
		                    <div class="mb-3">
		                        <label for="reason" class="form-label">품의 사유 및 상세 내용</label>
		                        <textarea class="form-control" name="reason" rows="5" placeholder="품의 사유 및 상세 내용을 입력하세요"></textarea>
		                    </div>
		                    <div class="mb-3">
		                        <label for="amount" class="form-label">예상 비용</label>
		                        <input type="number" class="form-control" name="amount" placeholder="예상 비용을 입력하세요">
		                    </div>
		                    <div class="mb-3">
		                        <label for="remarks" class="form-label">비고</label>
		                        <textarea class="form-control" name="remarks" rows="3" placeholder="비고를 입력하세요"></textarea>
		                    </div>
		                `;
		            } else if (selectedValue === '지출결의서') {
		                formHtml = `
		                	<h6>1. 작성자 정보</h6>
		                    <div class="mb-3">
		                        <label for="department" class="form-label">소속</label>
		                    ...
		                    <h6>2. 지출 내역</h6>
		                    <div class="mb-3">
		                        <label for="expenseReason" class="form-label">지출 사유 및 상세 내용</label>
		                        <textarea class="form-control" name="expenseReason" rows="4" placeholder="지출 사유 및 상세 내용을 입력하세요"></textarea>
		                    </div>
		                    <div class="mb-3">
		                        <label for="expenseAmount" class="form-label">지출 금액</label>
		                        <input type="number" class="form-control" name="expenseAmount" placeholder="지출 금액을 입력하세요">
		                    </div>
		                    <div class="mb-3">
		                        <label for="expenseDate" class="form-label">지출 일자</label>
		                        <input type="date" class="form-control" name="expenseDate">
		                    </div>
		                    <div class="mb-3">
		                        <label for="receipt" class="form-label">증빙 서류</label>
		    ...
		    function downloadPDF() {
		        const activeTab = document.querySelector('#modalContent');
		        if (activeTab) {
		            html2pdf().from(activeTab).save("결재문서.pdf");
		        }
		        else {
 					...
 					url:"/sign/createFormPost",
 		  			...
 		  			contentType:false,
 		   	    	...
 		   	    	success:function(result){
 						console.log("result : ", result);
 						// 등록
 							location.href="/sign/detail?appGroupCode="+appGroupCode;
 		   	    	}
 				});
